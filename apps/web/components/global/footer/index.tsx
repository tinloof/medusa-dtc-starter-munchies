...
interface FooterProps extends NonNullable<FooterType> {
...
export default function Footer({ variant = "default", ...props }: FooterProps) {
  ...
  return (
    <>
      <Newsletter {...props} />
      <footer className="w-full bg-accent" id="footer">
        <div className="mx-auto flex w-full max-w-max-screen flex-col gap-2xl px-m pt-6xl pb-m text-background lg:px-xl lg:pb-xl">
          <TopLinks {...props} />
          {props.image ? (
            <SanityImage className="lg:mt-2xl" data={props.image} />
          ) : null}
