...
export default async function subscribeNewsletterHandler({ event, container }: SubscriberArgs<{ id: string }>) {
  ...
  try {
    ...
    const response = await fetch(`${process.env.BACKEND_URL}/store/email/welcome/` + event.data.id, {
      method: "POST",
      headers: {
