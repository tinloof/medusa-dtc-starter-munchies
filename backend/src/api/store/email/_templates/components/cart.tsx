...
export default function Cart({
  ...
  return (
    <Section className="mb-10">
      ...
      {items.map((item) => {
        return <CartLine line={item} key={item.id} currency_code={currency_code} />;
      })}
      {details && (
        <Section className="max-w-[365px]" align="right">
          <CheckoutLine title="Sous-total" price={details.subtotal} />
          <CheckoutLine title="Rabais" price={details.discount} />
...
function CartLine({ line, currency_code }: { line: OrderLineItemDTO; currency_code: string }) {
    ...
    <Section className="mb-3">
      <Row>
        ...
        <Column className="pl-2 align-top ">
          <Text className="w-full font-bold pb-1 leading-[130%]" style={bodySmall}>
            {line.product_title}
          </Text>
          <Text className="w-full text-inactive leading-[120%]" style={bodySmall}>
            {line.variant_title}
          </Text>
        </Column>
...
function CheckoutLine({ title, subtitle, price }: { title: string; subtitle?: string; price: string }) {
  return (
    <Row className="mb-3">
